...
@Composable
fun SettingsAboutScreen(viewModel: SettingsAboutScreenViewModel = koinViewModel()) {
...
private fun GithubRepoContent(state: SettingsAboutScreenState) {
    ...
    val uri = state.githubRepoLink.asString()
    TaigaHeightSpacer(16.dp)
    TaigaOutlinedButton(
        painter = painterResource(RDrawable.github_mark),
        text = stringResource(RString.github_repo_link),
        onClick = {
...
private fun PrivacyPolicyContent(state: SettingsAboutScreenState) {
    val uriHandler = LocalUriHandler.current
    val uri = state.privacyPolicyLink.asString()
    TaigaHeightSpacer(16.dp)
    TaigaOutlinedButton(
        imageVector = Icons.Filled.Security,
        text = stringResource(RString.privacy_policy_title),
