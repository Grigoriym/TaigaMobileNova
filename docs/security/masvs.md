...
| MASVS-PLATFORM-3 | No `FLAG_SECURE` anywhere in the codebase (`grep -rn 'FLAG_SECURE' --include=*.kt .` empty; `MainActivity.kt` never calls `window.setFlags`) — the recents-list thumbnail can capture whatever is on screen, including a revealed password on `LoginScreen.kt:190-219` if the app is backgrounded mid-reveal | Requires the user to actively tap "show password" *and* background the app at that exact moment; the recents thumbnail is local to the device only (not synced/uploaded), so exploiting it needs physical/local access to an already-unlocked device | Explicit product decision, not an oversight: setting `FLAG_SECURE` on this single-`Activity` app would block screenshots/screen-recording app-wide (not just on login), which the app's maintainer does not want as a user of the app themselves. Weighed against a Low-Medium-severity, local-access-only gap on one field, the maintainer chose to keep screenshot capability over closing it. Not fixed, closes `docs/archive/revisit-resolved.md` #35 as a deliberate won't-fix |
...
| MASVS-AUTH-1 | **Android only** (iOS/JVM never reach this code — `isGithubOAuthSupported()` returns `false` there, button hidden). GitHub OAuth runs inside an in-app `WebView` (`GithubOAuthWebViewDialog.android.kt:22-44`) with `javaScriptEnabled = true` and `domStorageEnabled = true`, hosting GitHub's real login form under full app control — the RFC 8252 "embedded user-agent" anti-pattern, which fails regardless of configuration. Two bounds exist (no `addJavascriptInterface`/JS bridge anywhere in the codebase; the flow only executes on Android). One remaining gap beyond what the review plan assumed: navigation is not host-restricted (`shouldOverrideUrlLoading` only inspects for `code`/`error` query params, any other URL falls through to `return false` and loads). The other gap this row used to name — cookies never cleared on dismiss — is fixed (see Notes). This is a deliberate, documented tradeoff, not an oversight — a Custom Tabs + loopback-redirect version was built and reverted in the same PR (commit `4236a2ef`) because GitHub OAuth Apps support exactly one registered callback URL, already used by Taiga's web app; repointing it would break the web login, and a second OAuth App is a server-admin change outside this codebase. | `feature/login/ui/src/androidMain/kotlin/com/grappim/taigamobile/feature/login/ui/GithubOAuthWebViewDialog.android.kt:22-44`; reverted alternative documented (now marked superseded) in `docs/features/github-auth/plan.md` | Medium — full GitHub credentials are entered into a form the app controls; exploiting it needs the app itself compromised (malicious update) since there's no JS bridge today. Host-allowlisting the WebView navigation is explicit won't-fix — `docs/archive/revisit-resolved.md` #34 — because GitHub org SSO/SAML can redirect off `github.com` to a third-party IdP, that chain isn't enumerable from source, and a wrong allowlist could silently break login for org-SSO users with no device in this repo to verify against a real org SSO flow. Full RFC 8252 compliance is blocked on the external OAuth App constraint, not something this task can close. |
| MASVS-PLATFORM-2 | Same `GithubOAuthWebViewDialog.android.kt:22-44` code as the MASVS-AUTH-1 row above — this is the WebView-mechanics half of that finding (JS enabled, no navigation host-restriction), reviewed here as its own control rather than restated. No new gap found beyond what AUTH-1 already names. | See MASVS-AUTH-1 row above | Medium — same as MASVS-AUTH-1; tracked as one finding, not duplicated, host-allowlist won't-fix per `docs/archive/revisit-resolved.md` #34 |
...
| MASVS-AUTH-1 | Whether a real GitHub OAuth flow (including org SSO/2FA redirects) ever navigates the `WebView` to a host outside `github.com` before the `code`/`error` param appears — would be needed to scope a correct host allowlist, which `docs/archive/revisit-resolved.md` #34 records as won't-fix for now rather than a pending task | Needs a live GitHub OAuth login on a device against real org SSO configurations; not enumerable from source since GitHub controls the redirect chain. Only worth doing if this app needs to support org-SSO GitHub logins in practice — see `docs/archive/revisit-resolved.md` #34 |
...
- **iOS has no trust-manager equivalent at all — a functional gap, not a MASVS-NETWORK finding.**
  ...
  not misleading, just dead UI. Tracked as a follow-up (`docs/archive/revisit-resolved.md` #33), not fixed in this
  review.
- **Cleartext token exposure has a one-time warning at login, not an ongoing one.** `AuthHeaderPlugin`
  ...
  HTTP), with the remaining gap tracked as a follow-up (`docs/archive/revisit-resolved.md` #32, corrected this task)
...
- **MASVS-AUTH-1, GitHub OAuth WebView**: this is the finding named by the review plan's task 3 "Why."
  Confirmed with `file:line` (`GithubOAuthWebViewDialog.android.kt:22-38` at the time) and resolved as
  an Open finding rather than an Accepted deviation, because the two gaps beyond the plan's framing
  (navigation not host-restricted; `WebView` cookies never cleared on dismiss) were real and not yet
  bounded, even though the choice of `WebView` over Custom Tabs itself is a documented, deliberate
  tradeoff (GitHub OAuth Apps' single-callback-URL limit — see the Open table and `docs/archive/revisit-resolved.md` #34
  for the full history, including the reverted Custom Tabs/loopback implementation from the same
  original PR). The stale plan for that reverted approach, `docs/features/github-auth/plan.md`, is
  marked Superseded this task rather than left to be mistaken for the current design.
  **2026-08-13:** the cookie-clearing half is fixed — `GithubOAuthWebViewDialog.android.kt` now clears
  `CookieManager.getInstance().removeAllCookies(null)` from a `DisposableEffect(Unit)`'s `onDispose`,
  which fires whenever the dialog leaves composition (success or dismiss). Navigation host-restriction
  is explicit won't-fix, not fixed and not merely still open — see `docs/archive/revisit-resolved.md` #34 for why (GitHub
  org SSO/SAML can redirect off `github.com`, unverifiable from source with no device in this repo).
...
- **MASVS-PLATFORM-2, GitHub OAuth WebView**: same finding as MASVS-AUTH-1's Open row
  (`GithubOAuthWebViewDialog.android.kt:22-44`) — reviewed here for the WebView-mechanics control
  specifically (JS/DOM storage enabled, no host-restricted navigation; cookie-clearing is fixed, see
  the MASVS-AUTH-1 note above), cross-referenced rather than duplicated. Navigation host-restriction is
  won't-fix — see the Open table and `docs/archive/revisit-resolved.md` #34.
...
- **MASVS-PRIVACY-4, logout data clearing — fixed on all three platforms.** Android:
  ...
  **iOS fixed this task** (`core/storage/src/iosMain/.../db/TaigaDBExt.ios.kt:3`): the `= Unit` stub
  replaced with the same three `deleteAll()` DAO calls as the JVM actual — no new abstraction, the
  `expect`/DAO plumbing already existed. Verified: `:core:storage:compileKotlinIosSimulatorArm64`,
  `ktlintCheck` and the full `jvmTest`/`koverXmlReport`/`:koverVerify` all green (no automated iOS
  test infra in this repo to cover the actual itself, same as every other iOS-only class). Closes
  `docs/archive/revisit-resolved.md` #37.
- **MASVS-PLATFORM-3, `FLAG_SECURE` — deliberate won't-fix, not left open.** The maintainer, weighing
  the tradeoff as a user of the app themselves, chose to keep app-wide screenshot/screen-recording
  capability over closing a Low-Medium, local-access-only gap (revealed login password visible in a
  recents-list thumbnail if the app is backgrounded mid-reveal). Recorded as an Accepted deviation
  rather than an Open finding since it's a considered decision, not an oversight. Closes
...
- **MASVS-CODE-4, `LocalUriHandler.openUri()` scheme allowlist — fixed app-wide, not left open.** The
  "URL"-type custom field's open action (`CustomFieldUrlItemWidget`, was `CustomFieldsWidget.kt:452-464`)
  originally passed free-text collaborator input straight to `uriHandler.openUri()` with no scheme
  check — an implicit Android `ACTION_VIEW` intent with attacker-chosen data. The same risk class
  existed at two more call sites (`AttachmentsWidget.kt:160`; markdown-rendered links in task
  descriptions/comments/wiki, via `com.mikepenz:multiplatform-markdown-renderer`). Read that library's
  published sources (`AnnotatorSettings.kt`'s `annotatorSettings()`, `linkInteractionListener` default)
  to confirm what `docs/archive/revisit-resolved.md` #36 had only suspected: its default link-click handler calls
  `LocalUriHandler.current.openUri()` with no scheme check of its own — so the markdown path shares
  the exact same gap rather than having a separate one. Closed with the architectural fix #36 called
  for: `SafeUriHandler` (`uikit/.../utils/SafeUriHandler.kt`) wraps the ambient `UriHandler`, allowlisting
  `http://`/`https://` and refusing (logged, not launched) anything else; provided once via
  `CompositionLocalProvider` in `TaigaMobileTheme` (`uikit/.../theme/Theme.kt`), so every
  `LocalUriHandler.current` call app-wide — markdown links, `AttachmentsWidget`, the custom field, and
  any future call site — is covered without a per-call-site check. The custom field's own now-redundant
  `startsWith("http")` check was removed as part of the same change. Covered by
  `SafeUriHandlerTest` (`uikit` `commonTest`).
