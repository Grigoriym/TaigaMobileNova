...
| MASVS-AUTH-1 | Whether a real GitHub OAuth flow (including org SSO/2FA redirects) ever navigates the `WebView` to a host outside `github.com` before the `code`/`error` param appears — would be needed to scope a correct host allowlist, which `docs/revisit.md` #34 records as won't-fix for now rather than a pending task | Needs a live GitHub OAuth login on a device against real org SSO configurations; not enumerable from source since GitHub controls the redirect chain. Only worth doing if this app needs to support org-SSO GitHub logins in practice — see `docs/revisit.md` #34 |
...
- **MASVS-AUTH-1, GitHub OAuth WebView**: this is the finding named by the review plan's task 3 "Why."
  ...
  tradeoff (GitHub OAuth Apps' single-callback-URL limit — see the Open table and `docs/revisit.md` #34
  ...
  **2026-08-13:** the cookie-clearing half is fixed — `GithubOAuthWebViewDialog.android.kt` now clears
  ...
  is explicit won't-fix, not fixed and not merely still open — see `docs/revisit.md` #34 for why (GitHub
...
- **MASVS-PLATFORM-2, GitHub OAuth WebView**: same finding as MASVS-AUTH-1's Open row
  ...
  won't-fix — see the Open table and `docs/revisit.md` #34.
...
- **MASVS-CODE-4, `LocalUriHandler.openUri()` scheme allowlist — fixed app-wide, not left open.** The
  ...
  to confirm what `docs/revisit.md` #36 had only suspected: its default link-click handler calls
