# Revisit list — resolved entries (archive)

Entries #2–#44 from [docs/revisit.md](../revisit.md), split out here 2026-09-06 once that file
passed 2200 lines. Every entry below is resolved, moved elsewhere, or otherwise closed — kept for
its reasoning and evidence, not because it's actionable. ~20 links elsewhere in the repo point at
specific entries by anchor; moving the file here (rather than deleting these sections) is what
keeps those links working — see docs/revisit.md's own history for the original numbering.

Still-open items live in [docs/revisit.md](../revisit.md); nothing here needs a fresh look.
